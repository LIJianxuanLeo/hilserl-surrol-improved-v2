...
def make_robot_env(cfg: HILSerlRobotEnvConfig) -> tuple[gym.Env, Any]:
    ...
    if cfg.name == "gym_hil":
        assert cfg.robot is None, "GymHIL environment does not support robot config"
        ...
        if teleop_device is not None:
            ...
            from gym_hil.wrappers.hil_wrappers import GripperPenaltyWrapper, ResetDelayWrapper
            from gym_hil.wrappers.viewer_wrapper import PassiveViewerWrapper

            from lerobot.rl.ee_orientation_wrapper import (
                ActionSafetyWrapper,
            ...
            base_env = EEOrientationActionWrapper(
                base_env,
                ee_step_size={"x": 0.025, "y": 0.025, "z": 0.025},
                use_gripper=use_gripper,
            )
            base_env = ActionSafetyWrapper(base_env)
            base_env = PassiveViewerWrapper(base_env)
            env = ResetDelayWrapper(base_env, delay_seconds=1.0)
        else:
            env = gym.make(
                f"gym_hil/{cfg.task}",
                image_obs=True,
...
def control_loop(
    ...
    obs, info = env.reset()
    if hasattr(teleop_device, "reset_state"):
        teleop_device.reset_state()
    complementary_data = (
    ...
    while episode_idx < cfg.dataset.num_episodes_to_record:
        ...
        # Create a neutral action matching env action space dimensionality
        action_dim = env.action_space.shape[0] if hasattr(env.action_space, "shape") else 4
