...
class SACPolicy(
    ...
    def __init__(
        ...
        self._continuous_action_dim = continuous_action_dim
        self._init_action_normalization(config, continuous_action_dim)
        self._init_encoders()
        self._init_critics(continuous_action_dim)
        self._init_actor(continuous_action_dim)
        self._init_temperature()

    def _init_action_normalization(self, config: SACConfig, continuous_action_dim: int):
        """Register action min/max buffers for normalizing continuous actions to/from [-1,1]."""
        stats = config.dataset_stats
        if stats is not None and ACTION in stats:
            a_min = torch.tensor(stats[ACTION]["min"][:continuous_action_dim], dtype=torch.float32)
            a_max = torch.tensor(stats[ACTION]["max"][:continuous_action_dim], dtype=torch.float32)
        else:
            a_min = -torch.ones(continuous_action_dim)
            a_max = torch.ones(continuous_action_dim)
        self.register_buffer("_action_min", a_min)
        self.register_buffer("_action_max", a_max)

    def _normalize_actions(self, actions: Tensor) -> Tensor:
        """Map raw-scale continuous actions to [-1, 1]."""
        return 2.0 * (actions - self._action_min) / (self._action_max - self._action_min + 1e-8) - 1.0

    def _unnormalize_actions(self, actions: Tensor) -> Tensor:
        """Map [-1, 1] continuous actions to raw scale."""
        return (actions + 1.0) / 2.0 * (self._action_max - self._action_min) + self._action_min
    ...
    def select_action(self, batch: dict[str, Tensor]) -> Tensor:
        ...
        Returns actions in raw (unnormalized) scale so that they are consistent
        with human teleop actions that also live in raw scale.
        """
    ...
    def update_target_networks(self):
        ...
        for target_param, param in zip(
            self.encoder_target.parameters(), self.encoder_critic.parameters(), strict=True
        ):
            target_param.data.copy_(param.data * tau + target_param.data * (1.0 - tau))
        for target_param, param in zip(
            self.critic_target.critics.parameters(),
            self.critic_ensemble.critics.parameters(),
            strict=True,
        ):
            target_param.data.copy_(param.data * tau + target_param.data * (1.0 - tau))
        if self.config.num_discrete_actions is not None:
    ...
    def _init_critics(self, continuous_action_dim):
        ...
        self.encoder_target = copy.deepcopy(self.encoder_critic)
        if self.config.freeze_vision_encoder and self.encoder_target.has_images:
            freeze_image_encoder(self.encoder_target.image_encoder)

        target_heads = [
            CriticHead(
                input_dim=self.encoder_target.output_dim + continuous_action_dim,
                **asdict(self.config.critic_network_kwargs),
            )
            for _ in range(self.config.num_critics)
        ]
        self.critic_target = CriticEnsemble(encoder=self.encoder_target, ensemble=target_heads)
    ...
    def _init_actor(self, continuous_action_dim):
        ...
        if self.target_entropy is None:
            self.target_entropy = -continuous_action_dim
...
class SACObservationEncoder(nn.Module):
    ...
    def _init_normalization_buffers(self, config: SACConfig) -> None:
        """Register buffers for observation normalization (ImageNet for images, MIN_MAX for state)."""
        stats = config.dataset_stats
        if stats is None:
            self._norm_images = False
            self._norm_state = False
            self._norm_env = False
            return

        # Image normalization: MEAN_STD (ImageNet by default)
        self._norm_images = False
        if self.has_images:
            first_key = self.image_keys[0]
            if first_key in stats and "mean" in stats[first_key]:
                mean = torch.tensor(stats[first_key]["mean"], dtype=torch.float32).view(1, -1, 1, 1)
                std = torch.tensor(stats[first_key]["std"], dtype=torch.float32).view(1, -1, 1, 1)
                self.register_buffer("_img_mean", mean)
                self.register_buffer("_img_std", std)
                self._norm_images = True

        # State normalization: MIN_MAX → [-1, 1]
        self._norm_state = False
        if self.has_state and OBS_STATE in stats and "min" in stats[OBS_STATE]:
            s_min = torch.tensor(stats[OBS_STATE]["min"], dtype=torch.float32).unsqueeze(0)
            s_max = torch.tensor(stats[OBS_STATE]["max"], dtype=torch.float32).unsqueeze(0)
            self.register_buffer("_state_min", s_min)
            self.register_buffer("_state_max", s_max)
            self._norm_state = True

        # Env state normalization: MIN_MAX → [-1, 1]
        self._norm_env = False
        if self.has_env and OBS_ENV_STATE in stats and "min" in stats[OBS_ENV_STATE]:
            e_min = torch.tensor(stats[OBS_ENV_STATE]["min"], dtype=torch.float32).unsqueeze(0)
            e_max = torch.tensor(stats[OBS_ENV_STATE]["max"], dtype=torch.float32).unsqueeze(0)
            self.register_buffer("_env_min", e_min)
            self.register_buffer("_env_max", e_max)
            self._norm_env = True

    def _normalize_image(self, img: Tensor) -> Tensor:
        if self._norm_images:
            return (img - self._img_mean) / (self._img_std + 1e-8)
        return img

    def _normalize_state(self, state: Tensor) -> Tensor:
        if self._norm_state:
            return 2.0 * (state - self._state_min) / (self._state_max - self._state_min + 1e-8) - 1.0
        return state

    def _normalize_env(self, env_state: Tensor) -> Tensor:
        if self._norm_env:
            return 2.0 * (env_state - self._env_min) / (self._env_max - self._env_min + 1e-8) - 1.0
        return env_state
    ...
    def forward(
        ...
        if self.has_env:
            parts.append(self.env_encoder(self._normalize_env(obs[OBS_ENV_STATE])))
        if self.has_state:
            parts.append(self.state_encoder(self._normalize_state(obs[OBS_STATE])))
        if parts:
