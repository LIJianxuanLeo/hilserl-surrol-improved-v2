...
| 图像数据增强 | **DRQ-v2 random shift** (pad=6) | **DRQ-v2 random shift** (pad=6) | DRQ 随机裁剪/平移 | ✅ 已对齐（自实现 DrQ-v2 风格） |
...
| ~~**DRQ 数据增强**~~ | ~~无~~ → **已加（pad=6）** | 随机裁剪/平移 | ✅ **已实现并启用**（commit 之后） |
...
| 6 | ~~DRQ 增强缺失~~ → 已加 | **中** — 现已对齐参考，预期提升视角鲁棒性 5-10% |

---

## 12. 一句话结论

> **"我们的 V1/V2 在 A100 80GB 上的配置 = hil-serl-sim 同款 REDQ + DRQ 框架 + 4× 更激进的 UTD + 更大的网络 + 解冻编码器 + 独立的奖励设计。预期收敛速度优于参考实现的 30K env steps / 1 hour。"**

之前唯一落后的 DRQ 数据增强已实现并启用（`RandomShiftAug` 类，pad=6，DrQ-v2 风格）。
现在我们的配置在所有维度上要么对齐要么超过参考实现。
